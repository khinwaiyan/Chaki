...
export default function LoveLanguage() {
    ...
    useEffect(() => {
        const preloadImages = async (description) => {
            setIsLoading(true);
            try {
                ...
                console.error('Error preloading images:', error.response ? error.response.data : error.message);
                alert('Failed to preload images from the backend.');
            } finally {
                setIsLoading(false);
            }
        };

        if (selectLanguageDescription) {
            preloadImages(selectLanguageDescription);
        }
    ...
    const handleSendData = () => {
        ...
        navigate('/generate', { state: { imageUrls, language: selectLanguage, description: selectLanguageDescription } });
    };
        ...
        <Wrapper>
            ...
            <ContentWrapper>
                ...
                <ButtonGroup>
                    ...
                    <BtnWithBody title="스킨십" text="포옹, 손잡기 등 신체적 접촉을 통해 애정을 표현한다." onClick={() => handleLanguageSelection('스킨십', '포옹, 손잡기 등 신체적 접촉을 통해 애정을 표현한다.')} isSelected={selectLanguage === '스킨십'} />
                    <BtnWithBody title="인정하는 말" text="상대방에게 칭찬이나 긍정적인 말을 통해 사랑을 표현한다." onClick={() => handleLanguageSelection('인정하는 말', '상대방에게 칭찬이나 긍정적인 말을 통해 사랑을 표현한다.')} isSelected={selectLanguage === '인정하는 말'} />
                    <BtnWithBody title="함께하는 시간" text="질적으로 높은 시간을 함께 보내는 것을 중시한다." onClick={() => handleLanguageSelection('함께하는 시간', '질적으로 높은 시간을 함께 보내는 것을 중시한다.')} isSelected={selectLanguage === "함께하는 시간"} />
                </ButtonGroup>
